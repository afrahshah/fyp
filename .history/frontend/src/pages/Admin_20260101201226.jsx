...
export default function Admin() {
  const { contract, account, isAdmin, isIssuer, connectLocalDevAccount } = useWeb3();
          ...
          <div className="access-denied">
            ...
            {process.env.NODE_ENV !== 'production' && (
              <div style={{ marginTop: 16 }}>
                <p style={{ marginBottom: 8 }}>If you're running a local node, you can connect using the local deployer account (admin):</p>
                <button
                  className="btn btn-primary"
                  onClick={async () => {
                    const res = await connectLocalDevAccount();
                    if (res && res.ok) {
                      toast.success('Connected as local admin');
                    } else {
                      let msg = 'Local admin not available. Make sure your local node is running and the contract is deployed.';
                      if (res?.reason === 'no_contract') msg = 'No contract found at the configured address. Deploy the contract locally or update CONTRACT_ADDRESS in frontend config.';
                      if (res?.reason === 'not_admin') msg = 'Local account found but it is not an admin. Ensure you deployed contract with the expected deployer.';
                      if (res?.error) msg = res.error;
                      toast.error(msg);
                    }
                  }}
                >Use Local Admin</button>
              </div>
            )}
          </div>
        </div>
