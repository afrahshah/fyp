...
export function Web3Provider({ children }) {
  ...
  // Dev fallback: automatically connect to local node's first account (deployer)
  // This makes the frontend use the local deployer account (which is admin on localhost)
  useEffect(() => {
    const setupLocalDevAccount = async () => {
      ...
      try {
        ...
        // Read contract via provider and check if signerAddress is admin
        const contractWithProvider = new ethers.Contract(CONTRACT_ADDRESS, CONTRACT_ABI, localProvider);
        const adminStatus = await contractWithProvider.isAdmin(signerAddress);
        ...
        // If local node isn't running or doesn't have the contract, silently ignore
        // console.warn('Local dev fallback failed', err);
      }
    };
