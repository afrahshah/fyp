...
> For local development: the frontend will automatically connect to the local node's deployer account (admin) when MetaMask is not connected. In development the app will also auto-grant the deployer the Issuer role (if needed) so you can immediately issue certificates. You can also manually switch to the local admin via the Admin page by clicking **Use Local Admin** (only in non-production environments).
